...
def build_services() -> tuple[GenericApplicationService, RuntimeWorkspaceService]:
    profile = GenericApplicationProfile(
        runtime_journal_dir=tempfile.mkdtemp(prefix="cbi-webclient-")
    )
...
def test_build_webclient_runtime_state_contains_layout_and_runtime_snapshot() -> None:
    ...
    topology = application_service.load_topology(
        TEST_LAYOUT_PATH, load_runtime_state=False, load_occupancy=True
    )
...
def test_save_webclient_runtime_state_uses_atomic_replace(
...
def test_runtime_state_endpoint_returns_200_with_json(tmp_path: Path) -> None:
    ...
    with (
        running_server(runtime_state_path=runtime_state_path) as base_url,
        urllib.request.urlopen(f"{base_url}/api/runtime-state") as response,
    ):
